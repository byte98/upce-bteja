        ...
        public override void Execute()
        {
            if (this.expression != null)
            {
                ...
                if (this.expression.HasValue())
                {
                    ...
                    {
                        if (this.block.GetBlock().HasDefinition(this.varName) && this.block.GetBlock().GetDefinition(this.varName).GetTokenType() == TokenType.DT_NUMBER)
                        {
                            this.block.SetVariable(this.varName, this.expression.GetValue());
                        }
                        else
                        {
                            Interpreter.PrintError("Wrong data type of variable found in assignment execution!", this.block.GetBlock().GetDefinition(this.varName));
                        }
                        
                    }
                    else
                    {
                        Interpreter.PrintError("Unknown variable '" + this.varName + "' found in assignment execution!", this.token);
                    }
                }
                else
                {
                    Interpreter.PrintError("Failed to process expression for assignment!", this.token);
                }
            }
            else
            {
                Interpreter.PrintError("Execution of expression of assignment failed (nothing to execute)!", this.token);
            }
        }
    }
}
