        ...
        public override void Execute()
        {
            if (this.varName != null)
            {
                if (this.block.HasVariable(this.varName))
                {
                    ...
                    {
                        Interpreter.PrintError("Cannot execute read (invalid value)! Expected number!", this.token);
                    }
                }
                else
                {
                    Interpreter.PrintError("Cannot execute read (unknown variable '" + this.varName + "')!", this.token);
                }
            }
            else
            {
                Interpreter.PrintError("Cannot execute read (variable not defined)!", this.token);
            }
        }
    }
}
