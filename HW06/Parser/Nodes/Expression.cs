...
namespace HW06.Parser.Nodes
{
    ...
    class Expression : ExecutableNode
    {
        ...
        public override void Execute()
        {
            if (this.terms.Count > 0)
            {
                ...
                foreach(SignedTerm t in this.terms)
                {
                    ...
                    if (t.term.HasValue())
                    {
                        ...
                        {
                            Interpreter.PrintError("Cannot execute expression (term sign not defined)!", this.token);
                        }
                    }
                    else
                    {
                        Interpreter.PrintError("Cannot execute expression (term execution failed)!", this.token);
                    }
                }
                if (this.hasValue)
                {
            ...
            {
                Interpreter.PrintError("Cannot execute expression (terms not defined)!", this.token);
            }
        }
    }
}