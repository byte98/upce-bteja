...
namespace HW04.Parser.Nodes
{
    ...
    class Term : ExecutableNode
    {
        ...
        public override void Execute()
        {
                        ...
                        foreach(SignedFactor f in this.factors)
                        {
                            ...
                            if (f.factor.HasValue())
                            {
                                ...
                                else if (f.sign == SignedFactor.FactorSign.DIVIDE)
                                {
                                    ...
                                    {
                                        Parser.PrintError("Cannot execute term (division by factor with zero value)!", this.token);
                                    }
                                }
                                else
                                {
                                    Parser.PrintError("Cannot execute term (unknown factor sign)!", this.token);
                                }
                            }
                            else
                            {
                                Parser.PrintError("Cannot execute term (factor has no value)!", this.token);
                            }
                        }
                    }
                    this.hasValue = true;
            ...
            {
                Parser.PrintError("Cannot execute term (factor not defined)!", this.token);
            }
        }
    }
}
