...
{
    /// <summary>
    /// Class representing divide binary expression
    /// </summary>
    class DivideBinaryExpression : Node, Evaluatable
    {
        /// <summary>
        /// Left side of divide binary expression
        /// </summary>
        private LiteralExpression leftSide;

        /// <summary>
        /// Expression which is member of divide binary expression
        /// </summary>
        private Expression expression;

        /// <summary>
        /// Literal expression which is member of divide binary expression
        /// </summary>
        ...
        /// <param name="token">Token defining divide binary expression</param>
        public DivideBinaryExpression(Lexer.Lexer.LexerResults tokens, Lexer.Token token) : base(tokens, token) { }
            ...
            if (this.tokens.HasNext())
            {
                ...
                if (next.GetTokenType() == Lexer.TokenType.PLUS)
                {
                    if (this.token.GetTokenType() == Lexer.TokenType.NUMBER && this.token.HasValue())
                    {
                        ...
                        {
                            Parser.Print("Unexpected state during parsing divide binary expression! " + next.ToString());
                        }
                    }
                    else
                    {
                        Parser.Print("Unexpected left side of divide binary expression! " + this.token.ToString());
                    }
                }
                else
                {
        ...
        /// <summary>
        /// Builds left side of divide binary expression
        /// </summary>
