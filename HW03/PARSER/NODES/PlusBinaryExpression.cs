...
{
    /// <summary>
    /// Class representing plus binary expression
    /// </summary>
    class PlusBinaryExpression: Node, Evaluatable
    {
        /// <summary>
        /// Left side of plus binary expression
        /// </summary>
        private LiteralExpression leftSide;

        /// <summary>
        /// Expression which is member of plus binary expression
        /// </summary>
        private Expression expression;

        /// <summary>
        /// Literal expression which is member of plus binary expression
        /// </summary>
        ...
        /// <param name="token">Token defining plus binary expression</param>
        public PlusBinaryExpression(Lexer.Lexer.LexerResults tokens, Lexer.Token token) : base(tokens, token) { }
            ...
            if (this.tokens.HasNext())
            {
                ...
                if (next.GetTokenType() == Lexer.TokenType.PLUS)
                {
                    if (this.token.GetTokenType() == Lexer.TokenType.NUMBER && this.token.HasValue())
                    {
                        ...
                        {
                            Parser.Print("Unexpected state during parsing plus binary expression! " + next.ToString());
                        }
                    }
                    else
                    {
                        Parser.Print("Unexpected left side of plus binary expression! " + this.token.ToString());
                    }
                }
                else
                {
        ...
        /// <summary>
        /// Builds left side of plus binary expression
        /// </summary>
