...
{
    /// <summary>
    /// Class representing minus binary expression
    /// </summary>
    class MinusBinaryExpression: Node, Evaluatable
    {
        /// <summary>
        /// Left side of minus binary expression
        /// </summary>
        private LiteralExpression leftSide;

        /// <summary>
        /// Expression which is member of minus binary expression
        /// </summary>
        private Expression expression;

        /// <summary>
        /// Literal expression which is member of minus binary expression
        /// </summary>
        ...
        /// <param name="token">Token defining minus binary expression</param>
        public MinusBinaryExpression(Lexer.Lexer.LexerResults tokens, Lexer.Token token) : base(tokens, token) { }
            ...
            if (this.tokens.HasNext())
            {
                ...
                if (next.GetTokenType() == Lexer.TokenType.PLUS)
                {
                    if (this.token.GetTokenType() == Lexer.TokenType.NUMBER && this.token.HasValue())
                    {
                        ...
                        {
                            Parser.Print("Unexpected state during parsing minus binary expression! " + next.ToString());
                        }
                    }
                    else
                    {
                        Parser.Print("Unexpected left side of minus binary expression! " + this.token.ToString());
                    }
                }
                else
                {
        ...
        /// <summary>
        /// Builds left side of minus binary expression
        /// </summary>
